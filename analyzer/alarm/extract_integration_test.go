...
//
//	core.go → alarm.Parse(完整 bugreport)
//
// alarm.Parse 内部会自行调用 ExtractServiceDump 完成段提取，故生产路径是直接
// 把完整 dumpsys 喂给 Parse。本测试既复刻该调用，又单独守卫 ExtractServiceDump
// 接缝。fixture 使用已入库的 testdata/alarm_sample.txt。
func TestParse_ViaExtractServiceDump(t *testing.T) {
	b, err := os.ReadFile(filepath.Join("testdata", "alarm_sample.txt"))
	if err != nil {
		t.Fatalf("read alarm_sample.txt: %v", err)
	}
	raw := string(b)

	// 守卫 ExtractServiceDump 接缝。
	body := bugreportutils.ExtractServiceDump(raw, "alarm")
	if body == "" {
