...
const (
	...
	// Android 17 对应 reportVersion 36。已确认 wl 新增 bp 段、st 字段顺序变更。
	// 9 个新 section（rpm/gcf/ctf/fgs/awl/jbc/jbd/wmc/wmct）落入显式 case 容忍。
	maxParseReportVersion = 36
)
...
const (
	...
	videoData                   = "vid"
	audioData                   = "aud"
	// Android 17+ 新增 section，proto 暂无对应字段，显式容忍避免 "unknown data category" 警告。
	// 详见 frameworks/base/core/java/android/os/BatteryStats.java dumpCheckin。
	rpmData                  = "rpm"  // RPM stats（Resource Power Manager）
	gcfData                  = "gcf"  // Global Cpu Freq
	ctfData                  = "ctf"  // Cpu Time In Freq
	fgsData                  = "fgs"  // Foreground Service
	awlData                  = "awl"  // Attribute Wake Lock
	jbcData                  = "jbc"  // Job Battery Count
	jbdData                  = "jbd"  // Job Battery Duration
	wmcData                  = "wmc"  // Wakeup Module Count
	wmctData                 = "wmct" // Wakeup Module Cumulative Time
)
...
func parseSection(c checkinutil.Counter, reportVersion, rawUID int32, section string, record []string, app *bspb.BatteryStats_App, system *bspb.BatteryStats_System, apkSeen map[apkID]bool, allAppComputedPowerMah *float32) (bool, string, []error) {
	...
	case signalStrengthTimeData, signalStrengthCountData, dataConnectionTimeData, dataConnectionCountData, wifiStateTimeData, wifiStateCountData, bluetoothStateTimeData, bluetoothStateCountData, wifiSupplStateTimeData, wifiSupplStateCountData, wifiSignalStrengthTimeData, wifiSignalStrengthCountData:
		...
		return true, warn, nil
	case rpmData, gcfData, ctfData, fgsData, awlData, jbcData, jbdData, wmcData, wmctData:
		// Android 17+ 新增 section，proto 暂无对应字段。
		// 显式容忍避免 "unknown data category" 警告，数据丢弃。
		return true, "", nil
	default:
		return false, "", nil
	}
...
// 9,10007,l,st,10,20,30,40,50,60 (report version 17..35)
//   [top time, foreground service time, top sleeping time, foreground time, background time, cached time]
// 9,10007,l,st,10,20,30,40,50,60,70 (report version >= 36, Android 17+)
//   [top time, foreground service time, foreground time, background time, top sleeping time, heavy weight time, cached time]
//
// 注意：Android 17 (reportVersion 36) 调整了字段顺序——topSleep 从索引 3 移到索引 5，
// 新增 heavyWeight 在索引 6。proto 暂无 HeavyWeightTimeMsec 字段，heavyWeight 解析后丢弃。
func parseAppStateTime(pc checkinutil.Counter, record []string, reportVersion int32, app *bspb.BatteryStats_App) (string, []error) {
	...
	var err error
	if reportVersion >= 36 {
		// Android 17+: top, fs, fore, back, topSleep, heavyWeight, cached
		// heavyWeight 无 proto 字段，用匿名变量丢弃。
		var heavyWeight int64
		rem, err = parseSlice(pc, stateTimeData, record, &top, &foreServ, &fore, &back, &topSleep, &heavyWeight, &cached)
	} else {
		// 17..35: top, foreServ, topSleep, fore, back, cached
...
//
// format (v36+, Android 16+/17): 9,1000,l,wl,ActivityManager-Sleep,0,f,0,0,0,0,717,p,24,0,471,1893,1893,bp,24,0,471,1893,0,w,0,0,0,0
//   在 v21+ 基础上新增 "bp"（background partial）段，4 类 × 6 字段 = 24 数据字段。
//   proto 暂无 BackgroundPartial* 字段，bp 段解析后丢弃（避免污染后续 w 段）。
//   详见 frameworks/base/core/java/android/os/BatteryStats.java dumpCheckinLocked 中的
//   BACKGROUND_PARTIAL_WAKE_LOCK 段（"bp"）输出。
func parseAppWakelock(c checkinutil.Counter, reportVersion int32, record []string, app *bspb.BatteryStats_App) (string, error) {
	...
	} else if reportVersion >= 36 {
		// Android 16+/17 (rv36): 4 类 wakelock（full/partial/backgroundPartial/window），
		// 每类 6 字段（time, type, count, cur, max, tot）= 24 数据字段。
		// proto 无 BackgroundPartial* 字段，bp 数据解析后丢弃（仅避免污染 w 段）。
		var bpt, bpc float32
		var bpCur, bpMax, bpTot int64
		rem, err = parseSlice(c, wakelockData, record, &name,
			&ft, nil /*"f"*/, &fc, &fCur, &fMax, &fTot,
			&pt, nil /*"p"*/, &pc, &pCur, &pMax, &pTot,
			&bpt, nil /*"bp"*/, &bpc, &bpCur, &bpMax, &bpTot,
