...
> **实施状态（2026-07-19 校准）**：可行性已得到代码验证。P1（Form A 独立 module + HTTP 代理）、P2（Form B 原生 `--mcp` 嵌入）、P3-B（chart/report HTML）、P3-C（健康度评分）**已于 commit 8a271c6 移除**。本评估文档保留为「设计阶段归档 + 现状对照」用途，最新事实以本文「实施状态」小节与 `MCP概要设计.md` 为准。**import 路径 bug 已修复**：6 个误用 `github.com/google/battery-historian/...` 旧路径的文件已批量替换为 `github.com/reedhoop/ai-battery-historian/...`，`go build ./...` / `go vet ./analyzer/... ./cmd/battery-historian/...` / `go test ./analyzer/...` 全部通过。同时完成 code review 发现的 P0/P1 安全加固（base64 预检 / 路径沙箱 / prompt 注入防护 / Compare `report_index`）（P2/P3 健康度优化随功能移除而不再适用），详见 §六.6 与 §十二。

---

## ⚠️ 功能移除声明（2026-07-19）

> 自定义电池**健康度评分**功能（原 P3-C）已于 commit `8a271c6` 整体移除：含 `analyzer/health` 包、`query_health` tool、`bugreport://{id}/health` resource、`battery_health_report` prompt、`cmd/healthcheck` CLI 与结果页健康度卡片。用户认为其对问题分析实际意义不大。**原生 batterystats `HistogramStats`（FR-05 `query_histogram`，即「健康度直方图」）、System Health 可视化 tab、`metrics.js` 的 `HEALTH` metric 均保留不受影响。** 本文档凡标注「P3-C 健康度 / query_health / health resource / battery_health_report / analyzer/health / cmd/healthcheck」之处，均指向**已移除**代码，仅作历史归档，请勿据此对接。

---

## 一、一句话结论

**已验证可行，且收益已部分兑现。** Battery Historian 的核心解析管线（checkin 解析、history 汇总、app 级统计、A/B 对比）全部是**纯 Go**，与前端 JS 完全解耦，已通过两条路径封装为 MCP 工具：(1) Form A 独立 module（`mcp-server/`）走 HTTP 代理；(2) Form B 原生嵌入（`cmd/battery-historian --mcp`）进程内直调 `analyzer.Analyze/Compare`。P3-B 已用纯 Go `generateV2ChartSVG` 实现 Format:2 报告的电量曲线图（fallback 自包含 SVG），P3-C 原用 `analyzer/health` 包实现 6 维度健康度评分**已于 commit 8a271c6 移除**。import 路径 bug 已修复，`go build ./...` / `go vet` / `go test ./analyzer/...` 全部通过；并完成 code review 发现的 P0/P1 安全加固（base64 预检 / 路径沙箱 / prompt 注入防护 / Compare `report_index`）（P2/P3 健康度优化随功能移除而不再适用）。
...
| 构建系统 | **已加 `go.mod`**（module `github.com/reedhoop/ai-battery-historian`，go 1.25.5），全仓 import 路径已统一（P3-B/C 新增的 6 个文件中，`analyzer/health/*` 与 `cmd/healthcheck/*` 已于 commit 8a271c6 随健康度功能移除），`go build ./...` / `go vet` / `go test ./analyzer/...` 全部通过 | 主体已模块化，无遗留问题 |
...
- `HistogramStats`：屏幕/信号/移动网络/蓝牙/Modem 等各项百分比与每小时速率，正好适合 AI 做「健康度评分」（注：基于该结构的自定义健康度评分功能已于 commit 8a271c6 移除，原生 `HistogramStats` 数据本身保留）。
...
| ~~`query_health(id)`~~ **已移除（commit 8a271c6）** | 原 `AnalysisResult.Health`（`*health.Report`，P3-C） | ❌ 已移除 |
...
- ~~`bugreport://{id}/health`~~ **已移除（commit 8a271c6）** → 原 `AnalysisResult.Health` JSON（P3-C，Form B only）

### Prompts（给 AI 的提示词模板）
- `battery_root_cause`：给定系统/应用指标，输出根因分析框架。（Form A + B）
- `battery_ab_report`：给定 A/B diff，输出升级前后耗电变化结论。（Form A + B）
- ~~`battery_health_report`~~ **已移除（commit 8a271c6）**：原给定健康度报告，输出解读与改进建议。（P3-C，Form B only）
...
6. **~~P3-B/P3-C 新增文件 import 路径 bug~~ → 已解决**
   P3-B/P3-C 阶段新增的 6 个 Go 文件曾误用 `github.com/google/battery-historian/...` 旧 import 路径，与 `go.mod` 声明的 `github.com/reedhoop/ai-battery-historian` 模块路径冲突，且 `go.mod` 无 replace 兜底。Go 工具链会去公网拉取上游 `github.com/google/battery-historian`，但上游缺 `analyzer/health` 子包（已于 commit 8a271c6 随功能移除）、缺 `AnalyzeWithChart` 函数、缺 `AnalysisResult.PlotHTML/ReportHTML/Health`（Health 字段已随功能移除）字段，导致符号解析失败。
   **受影响文件**（已修复）：
   - `analyzer/core.go`（import aggregated / ~~analyzer/health~~ / pb/batterystats_proto / presenter，analyzer/health 已于 commit 8a271c6 移除）
   - ~~`analyzer/health/health.go`~~（已于 commit 8a271c6 移除）
   - ~~`analyzer/health/health_test.go`~~（已于 commit 8a271c6 移除）
...
7. **【Code Review 修复，2026-07-19】P0/P1 安全加固（P2/P3 健康度优化随功能移除不适用）**
   ...
   - **Health 全无效得 0 分**（⚠️ 已随功能于 commit 8a271c6 移除，仅作历史归档）（`health.go`）：所有维度 `Valid=false` 时返回 `Score=0 / Grade="F"`，误导用户。已改为返回 `Grade="N/A"` + 说明性 Summary。
   - **NaN/Inf 穿透**（⚠️ 已随功能于 commit 8a271c6 移除，仅作历史归档）（`health.go`）：`lerpDown`/`lerpUp`/`Evaluate` 未防护 NaN/Inf 输入。已加 `isFinite` helper（`!math.IsNaN && !math.IsInf`）多重防护。
   - **各维度无数据判定缺失**（⚠️ 已随功能于 commit 8a271c6 移除，仅作历史归档）（`health.go`）：5 个维度在数据缺失时仍按 0 分参与加权。已为 `wakelock_burden` / `wakeup_sync_freq` / `app_stability` / `doze_adoption` / `modem_activity` 增加 `Valid=false` 分支。
   ...
   - **【P2】alert 单位**（⚠️ 已随功能于 commit 8a271c6 移除，仅作历史归档）（`health.go buildAlerts`）：原 `Value` 字段为 `fmt.Sprintf("%.1f", ...)` 无单位。已加 `alertValue` helper，按维度返回 `%/h` / `%` / `次/h` / `次`。
   - **【P2】alert 二级排序**（⚠️ 已随功能于 commit 8a271c6 移除，仅作历史归档）（`health.go buildAlerts`）：原仅按 level 排序，同 level 内无序。已加 score 升序二级排序，同 severity 内更差者先出。
   - **【P3】summarize worst 排序**（⚠️ 已随功能于 commit 8a271c6 移除，仅作历史归档）（`health.go summarize`）：原 worst 列表按维度声明顺序输出。已改为按 score 升序，扣分最严重的维度先出。
   - **验证**：`go build ./...` ✅；`go vet ./analyzer/... ./cmd/battery-historian/...` ✅；`go test ./analyzer/...` ✅（`analyzer` 全绿；`analyzer/health` 已于 commit 8a271c6 随功能移除）。
...
- 补全 9 个 tool + 6 个 resource + 2 个 prompt（battery_health_report 已于 commit 8a271c6 移除）+ Top-N 分页。
...
- **P3-C（❌ 已移除，commit 8a271c6）**：原 `analyzer/health/health.go:81 Evaluate` 实现 6 维度加权健康度评分，通过 `query_health` tool + `bugreport://{id}/health` resource + `battery_health_report` prompt 暴露（现已整体删除）。
...
| Phase 3-C 健康度评分 | 未评估 | ❌ 已移除（commit 8a271c6） | 原 `analyzer/health` 6 维度加权 |
| Phase 3-A Python 3 迁移 | 2~3 人天 | ⏸ 未做 | P3-B 已绕开需求 |
| Code Review 修复 | 未评估 | ✅ 已完成（2026-07-19） | P0/P1 安全加固（P2/P3 健康度优化随功能移除不适用），详见 §六.7 |

**总评**：技术可行性已得到代码验证；A/B 两条 MCP 路径都已落地，P3-B 增值能力已实现（P3-C 健康度评分已于 commit 8a271c6 移除），import 路径 bug 已修复，code review 发现的 P0/P1 安全问题全部修复（P2/P3 健康度相关随功能移除不适用），`go build ./...` / `go vet` / `go test ./analyzer/...` 全绿。
...
- 既有 CLI：`cmd/checkin-parse`、`cmd/history-parse`、`cmd/checkin-delta`（~~`cmd/healthcheck`~~ 已于 commit 8a271c6 随健康度功能移除）。
...
- **新增（P3-C）健康度评分**（❌ 已于 commit 8a271c6 移除，仅作历史归档）：原 `analyzer/health/health.go:81 Evaluate`、`health.go:59 Report`、6 维度函数 `health.go:160/189/206/223/240/257`。
- **新增（P2）MCP server**：`cmd/battery-historian/mcp.go`（Form B，9 tools/6 resources/2 prompts（battery_health_report 已于 commit 8a271c6 移除））、`cmd/battery-historian/mcp_store.go`（LRU Store）、`mcp-server/`（Form A 独立 module）。
