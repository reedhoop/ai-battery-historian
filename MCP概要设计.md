...
> **实施状态（2026-07-19 校准）**：设计已全部落地。Analysis Core 实现于 `analyzer/core.go`；MCP Server 有两套并行实现：Form A（`mcp-server/` 独立 module，HTTP 代理）与 Form B（`cmd/battery-historian --mcp`，进程内直调 Core）。P3-B（chart/report HTML via `chart_v2.go`）已实现；P3-C（健康度评分 via `analyzer/health`）**已于 commit `8a271c6` 移除**。本文档原"设计建议"语气已改为"已实施描述"，§3.1 契约对齐 `core.go` 实际签名，§5 能力清单对齐实际注册的 tool/resource/prompt。**import 路径 bug 已修复**：6 个误用 `github.com/google/battery-historian/...` 旧路径的文件已批量替换为 `github.com/reedhoop/ai-battery-historian/...`，`go build ./...` / `go vet` / `go test ./analyzer/...` 全部通过；同步完成 code review 发现的 P0/P1 安全加固（详见 §12.3）；P2/P3 健康度优化随功能移除而不再适用。
...
| 最小侵入 | 不修改现有解析库函数语义；仅做"提取/封装"，必要时新建 facade | ✅ 仅新增 `core.go` / `chart_v2.go`（P3-C 的 `health/` 子包已于 commit `8a271c6` 移除），未改 `parseBugReport` |
...
type AnalysisResult struct {                                         // core.go:33
    ...
    ReportHTML string       // P3-B：自包含分析报告 HTML（图表 + 关键聚合；健康度卡片已于 commit 8a271c6 移除）
    // P3-C 的 Health *health.Report 字段已于 commit 8a271c6 随功能整体移除
}
...
6. 新增 P3-B 字段 `PlotHTML` / `ReportHTML`（P3-C 的 `Health` 字段已于 commit 8a271c6 移除），原设计未规划 P3-B。
...
- `analysisResults`（core.go:131）从 `pd.data` / `pd.responseArr` 组装 `AnalysisResult`（P3-C 的 `health.Evaluate` 填充已于 commit 8a271c6 移除）。
...
| **B. 原生 `--mcp` 进程内直调 Core** | `cmd/battery-historian/mcp.go` + `mcp_store.go`（主仓 `github.com/reedhoop/ai-battery-historian`，v0.3.0） | **不调**（`ParsedData.skipPlot=true` 跳过 `generateHistorianPlot`） | `analyzer.AnalysisResults` 全量（含 `aggregated.Checkin`、`[]AppStat`、`HistogramStats`、`RawCheckin`、`PlotHTML`、`ReportHTML`（P3-C 的 `Health` 已于 commit 8a271c6 移除），P4 起追加 `PowerSummary`/`AlarmSummary`/`ActivityStats`/`ProcStats`） | 13 / 10 / 3 | ✅ 已交付 |
...
AI → Tool.analyze_bugreport(path|content)
   ...
   → postProcess: PlotHTML fallback (generateV2ChartSVG) + ReportHTML (generateReportHTML)
...
AI → Tool.query_system_stats(id) / query_app_stats(id, uid?, topN?) / query_histogram(id)
       / query_wakelocks(id, kind?) / query_wakeup_reasons(id) / query_sync_tasks(id)
...
| `query_health` | `{id, report_index?}` | ~~`*health.Report`（P3-C）~~ **已移除（commit 8a271c6）** | 原 `AnalysisResult.Health` | ❌ 已移除 |
...
| `bugreport://{id}/health` | ~~健康度评分 JSON（P3-C）~~ **已移除（commit 8a271c6）** | 原 `*health.Report` | ❌ 已移除 |
...
| `battery_health_report` | ~~健康度报告~~ **已移除（commit 8a271c6）** | 原解读与改进建议（P3-C） | ❌ 已移除 |
...
- **Core 层 `AnalysisResult`**（`analyzer/core.go:33`，已实现，字段详见 §3.1）：含 `Checkin` / `AppStats` / `HistogramStats` / `RawCheckin` + 元数据 + P3-B `PlotHTML`/`ReportHTML`（P3-C `Health` 已于 commit 8a271c6 移除）。
...
- **P3-C 健康度报告 ❌ 已移除（commit 8a271c6）**（`analyzer/health/health.go:59`，现已删除）：
  ```go
  type Report struct {
...
- **P3-B 图表 HTML**：`AnalysisResult.PlotHTML` 有两种填充路径——(a) `AnalyzeWithChart` 走 Python `scripts/historian.py`（需 `--mcp_with_chart`）；(b) `Analyze`/`Compare` 在 `postProcess` 中 `PlotHTML == ""` 时调 `generateV2ChartSVG`（`chart_v2.go:119`，纯 Go，仅 Format:2 报告）生成自包含 inline-SVG 电量曲线。`ReportHTML` 由 `generateReportHTML`（`chart_v2.go:301`）始终填充，含设备元数据 + 电量曲线 + 关键聚合统计（健康度卡片已于 commit 8a271c6 移除）。
...
| 新增 P3-C 健康度评分 | 中 | ❌ `analyzer/health/health.go` 已于 commit 8a271c6 移除（原落地的 `Evaluate` + 6 维度评分随之删除） |
...
| **Code Review 修复（2026-07-19）** | 中 | P0/P1 安全加固全部修复：base64 预检 / 路径沙箱 / prompt 注入防护 / Compare `report_index` / alert 单位 + 二级排序（详见 §12.3）。P3 健康度评分相关优化（Health 全无效 N/A + NaN/Inf 防护 / 5 维度无数据判定）随功能于 commit 8a271c6 移除而不再适用。 |
...
- **P3-C（健康度评分）❌ 已移除（commit 8a271c6）**：原 `analyzer/health/health.go` 落地 `Evaluate` + 6 维度评分，通过 `query_health` tool + `bugreport://{id}/health` resource + `battery_health_report` prompt 暴露（**现已整体删除**）。
...
| **~~P3-B/P3-C 新增文件 import 路径 bug~~** | 把 6 个文件中 `github.com/google/battery-historian/...` 批量替换为 `github.com/reedhoop/ai-battery-historian/...`（其中 `analyzer/health/*` 与 `cmd/healthcheck/*` 已于 commit 8a271c6 随健康度功能移除而删除） | ✅ 已修复，`go build ./...` 通过 |
...
| **【2026-07-19 修复】Compare B 报告无法查询 / alert 单位与排序** | 全部 query_* 工具加 `report_index`（含已移除的 `query_health`；现 10 个：P4 新增 4 个，query_health 已移除）；`buildAlerts` 加 `alertValue` 单位 + score 二级排序 | ✅ 已修复（Health 全无效项随功能移除不适用） |
...
1. **§3.1 Core 契约对齐**：原设计 `Analyze([]byte, Options) (*AnalysisResult, error)` → 实际 `Analyze(string) (AnalysisResults, error)`；`Compare` 返回 `*CompareResult`（包装 `Combined`）而非裸 `*CombinedCheckinSummary`；无 `Options` 类型，改用 `ParsedData.skipPlot` 字段；字段名 `SystemStats→Checkin`、`Histogram→HistogramStats`、`rawCheckin→RawCheckin`（已导出）；新增 P3-B `PlotHTML/ReportHTML`（P3-C 的 `Health` 字段已于 commit 8a271c6 移除）；无 `ID` 字段（id 在 MCP 层维护）。
...
4. **§5 能力清单补全**：补 P3-B `chart`/`report` resource（P3-C 的 `query_health` tool / `health` resource / `battery_health_report` prompt 已于 commit 8a271c6 移除）；标注 Form A/B 各自实现范围。
5. **§6 数据结构补全**：补 `health.Report` 结构与 6 维度权重（⚠️ 已随功能于 commit 8a271c6 移除，仅作历史归档）；补 `PlotHTML` 两种填充路径。
...
7. **新增阻塞项**：P3-B/P3-C 新增的 6 个文件（`analyzer/core.go` / `analyzer/health/health.go` / `analyzer/health/health_test.go` / `cmd/battery-historian/mcp.go` / `cmd/battery-historian/mcp_store.go` / `cmd/healthcheck/main.go`，其中 `analyzer/health/*` 与 `cmd/healthcheck/*` 已于 commit 8a271c6 随功能移除）误用 `github.com/google/battery-historian/...` 旧 import 路径，与 go.mod 模块路径冲突，`go build ./...` 失败。
...
4. **§5.1 query_* 工具新增 `report_index` 参数**：全部 query_* 工具（原 7 个含已移除的 `query_health`；现 10 个：P4 新增 4 个，query_health 已移除）增加可选 `report_index`（默认 0=A，1=B），`resultForID` / `reportIndexFromReq` helper 校验范围；A/B 对比的 B 报告现在可被任意 query_* 工具访问。
5. **§5.3 Prompts 注入防护**：两个 prompt handler（`battery_root_cause` / `battery_ab_report`；`battery_health_report` 已于 commit 8a271c6 移除）的 user-supplied 参数改用 `wrapUserData` 包装为 `<user_data>...</user_data>`，并在 prompt 前置 `promptInjectionGuard` 声明该标签内为不可信 DATA。
6. **§6 `health.Report` 评分逻辑加固（❌ 已随功能于 commit 8a271c6 移除，仅作历史归档）**：
...
7. **§8 安全（NFR-04）加固**：`loadInput` 增加 (a) base64 **编码长度预检**（`maxEncodedLen = maxFileSize*4/3 + 4`）避免解码 DoS；(b) `filepath.Clean` 规范化 + `fi.Mode().IsRegular()` 守卫，拒绝目录 / 设备文件 / socket / 管道；(c) 三个 prompt handler 加 `wrapUserData` + `promptInjectionGuard` 注入防护；(d) Compare B 报告 `report_index` 访问（全部 query_* 工具增加 `report_index`，`reportIndexFromReq` 校验）。
8. **§6 `health.Alert` 输出优化（❌ 已随功能于 commit 8a271c6 移除，仅作历史归档）**：`buildAlerts` 新增 `alertValue` helper 按维度返回带单位值（`%/h` / `%` / `次/h` / `次`）；排序从单 level 改为 level + score 升序二级排序，同 severity 内更差者先出。
9. **§6 `summarize` worst 排序（❌ 已随功能于 commit 8a271c6 移除，仅作历史归档）**：worst 列表按 score 升序，扣分最严重的维度先输出。
